...
- 新增运维与监控基础设施：
  - 前端错误监控脚本（`scripts/error-tracking.mjs`）：捕获 JavaScript 运行时错误、Promise rejection，支持上报到 webhook
  - 构建产物大小报告（`scripts/build-report.mjs`）：构建完成后输出资源分类统计与大文件列表
  - 备份脚本（`scripts/backup.mjs`）：支持本地/GitHub Releases 备份，配置定时自动执行
  - 回滚脚本（`scripts/rollback.sh`）：一键回滚到上一个版本
  - Playwright 视觉回归测试（`tests/`, `playwright.config.mjs`）：覆盖首页、博客、导航、主题切换、可访问性等场景
...
- 新增文章草稿预览流程：`draft: true` 的文章会继续生成详情页并显示草稿提示，但默认不会进入文章列表、RSS、sitemap 和搜索引擎收录。
- 首页"内容模块"与精选项目支持直接从关于页、项目页、文章与近况数据自动生成摘要、数量与跳转入口，减少重复维护。
- 新增内容统计页：自动汇总公开文章的篇数、字数、阅读体量、分类、标签、模板、系列与更新时间，并生成 `/blog/stats/` 页面。
...
- README 新增"发布流程"章节，详细说明新增内容、部署流程、回滚操作、备份策略与测试流程。
